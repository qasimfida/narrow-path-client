  ...
  return (
    <div className={classes.root}>
      {search.map((item, index) => {
        return (
          <div key={item.name} onClick={()=>{handleActive(index)}} className={ (state[index].active ? classes.active : '' ) + " " + classes.searchItem }>
            <h3 className={classes.searchItemTitle}>{item.name}</h3>
            <input className={classes.searchItemInput} placeholder={item.placeHolder} onChange={(e)=> handleActive(index, e)}  ></input>
          </div>
        );
      })}
      <div className={classes.searchBtn} onClick={handleSearch} >h</div>
    </div>
  );
};
